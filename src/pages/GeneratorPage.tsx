    ...
    useEffect(() => {
        ...
        if (saved) {
            try {
                ...
                // Bump old saved dates to current year so picker shows latest
                if (parsed.date && parsed.date.slice(0, 4) !== String(currentYear)) {
                    parsed.date = getDefaultDate();
                }
                if (parsed.dueDate && parsed.dueDate.slice(0, 4) !== String(currentYear)) {
                    parsed.dueDate = getDefaultDueDate();
                }
    ...
    return (
        <div className="bg-[#F8FAFC] font-sans text-slate-900 flex flex-col relative screen-layout-only print:h-auto print:overflow-visible">
            ...
            <div className="flex-1 flex overflow-hidden z-10 relative print:overflow-visible print:block">
                                ...
                                {activeSection === 'details' && (
                                    <div className="p-4 space-y-4 border-t border-slate-100 animate-in slide-in-from-top-2 duration-200">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <DatePicker label="Date" value={data.date} onChange={(v) => handleInputChange('date', v)} placeholder="Invoice date" />
                                            <DatePicker label="Due Date" value={data.dueDate} onChange={(v) => handleInputChange('dueDate', v)} min={data.date} placeholder="Due date" />
                                        </div>
                                    </div>
                                )}
                            </div>
                ...
                {/* ==================================================================================== */}
                <div className={`preview-wrapper w-full lg:w-7/12 xl:w-2/3 bg-slate-100/90 relative flex-col items-center lg:flex ${mobileView === 'editor' ? 'hidden' : 'flex'}`}>
                    <div className="absolute inset-0 [background-image:radial-gradient(circle_at_1px_1px,rgba(148,163,184,0.12)_1px,transparent_0)] [background-size:20px_20px] pointer-events-none" aria-hidden />
                    <div className="w-full flex justify-center p-0 pb-32 lg:p-8 relative z-10">
                        <div id="invoice-preview-container" className="bg-white shadow-[0_4px_24px_rgba(0,0,0,0.08),0_0_1px_rgba(0,0,0,0.06)] rounded-lg border border-slate-200/80 w-full max-w-[794px] h-auto relative flex flex-col transition-all duration-300">
                            {/* TEMPLATES */}
