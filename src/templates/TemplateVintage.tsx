...
export const TemplateVintage: React.FC<TemplateProps> = ({ data, subtotal, taxAmount, total, isIGST }) => {
    return (
        ...
        <div className="w-full overflow-x-auto bg-gray-100 p-4">
            ...
            {/* 2. INVOICE CONTAINER: Enforce A4 Width (approx 794px) and Aspect Ratio */}
            {/* min-w-[794px] prevents the columns from getting squished */}
            <div className="font-sans text-black p-8 flex flex-col bg-white text-[11px] leading-normal mx-auto shadow-lg box-border min-w-[794px] max-w-[794px] min-h-[1123px]">
                    ...
                    <div className="flex border-b border-slate-400">
                        ...
                        <div className="w-1/2 flex flex-col">
                            ...
                            <div className="flex flex-grow">
                                {/* Added break-words to prevent spillover */}
                                <div className="w-1/2 p-2 border-r border-slate-400 border-b border-slate-400 flex flex-col justify-center break-words">
                    ...
                    <div className="flex-grow flex flex-col">
                        ...
                        {data.items.map((item, idx) => (
                            <div key={item.id} className="flex border-b border-slate-200 text-[10px] min-h-[45px]">
                                ...
                                <div className="w-[12%] py-2 text-right px-3 border-r border-slate-400 font-bold flex items-center justify-end whitespace-nowrap">{formatCurrency(item.rate).replace('₹', '')}</div>
                                <div className="w-[8%] py-2 text-right px-3 border-r border-slate-400 flex items-center justify-end">{item.quantity}</div>
                                <div className="w-[15%] py-2 text-right px-3 border-r border-slate-400 flex items-center justify-end whitespace-nowrap">{formatCurrency(item.rate * item.quantity).replace('₹', '')}</div>
                                <div className="w-[15%] py-2 text-right px-3 font-bold flex items-center justify-end whitespace-nowrap">{formatCurrency(item.rate * item.quantity).replace('₹', '')}</div>
                            </div>
                        ))}
                        {/* Fill empty space */}
