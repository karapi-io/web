...
export const TemplateService: React.FC<TemplateProps> = ({ data, subtotal, taxAmount, total, isIGST }) => {
    return (
        ...
        <div className="w-full overflow-x-auto bg-gray-100 p-4">
            ...
            <div className="font-sans text-slate-800 p-8 flex flex-col bg-white text-[11px] leading-snug mx-auto shadow-lg box-border min-w-[794px] max-w-[794px] min-h-[1123px]">
                ...
                {/* 3. ITEMS TABLE */}
                {/* FIXED: Removed 'flex-grow' so it doesn't push content down */}
                <div className="mb-6">
                        ...
                        <tbody>
                            {data.items.map((item, idx) => (
                                <tr key={item.id} className="border-b border-slate-100 last:border-0">
                                    ...
                                    {/* Added whitespace-nowrap to prevent currency wrapping */}
                                    <td className="py-2 align-top text-right whitespace-nowrap">{formatCurrency(item.rate).replace('₹', '')}</td>
                                    <td className="py-2 align-top text-right">{item.quantity}</td>
                                    <td className="py-2 align-top text-right font-bold whitespace-nowrap">{formatCurrency(item.rate * item.quantity).replace('₹', '')}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                {/* 4. TOTALS & BANK */}
                {/* Now flows immediately after the table */}
